...
| Road topology and island access | [`src/island_analysis.py`](../src/island_analysis.py), [`src/utils.py`](../src/utils.py) | Filters the disrupted road graph, computes connected-component islands, maps assets to islands and access-point identifiers (`access_rfid`), and redistributes island-based crews. |
